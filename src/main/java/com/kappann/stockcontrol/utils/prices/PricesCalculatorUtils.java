    ...
    public static BigDecimal getCostPriceFromComponents (List<ProductComponent> components) {
        ...
        return components
                ...
                .map(productComponent -> {
                    ...
                    return isComponentNullOrMissingValues(productComponent, product) ? BigDecimal.ZERO : multiplyUnitaryCostByQuantity(productComponent, product);
                })
                .reduce(BigDecimal::add)
                .orElse(BigDecimal.ZERO);
    }

    private static BigDecimal multiplyUnitaryCostByQuantity (ProductComponent productComponent, Product product) {
        return product.getCostPrice().multiply(BigDecimal.valueOf(productComponent.getQuantity()));
    }

    private static boolean isComponentNullOrMissingValues (ProductComponent productComponent, Product product) {
        return product == null || product.getCostPrice() == null || productComponent.getQuantity() == null;
    }
